...
from bdpan_over.v3.dataset import OverTestDataset
...
def build_data():
    test_dataset = OverTestDataset(src_image_dir, 0.5)
...
def build_model():
    restore_model = OverRestoreModelV3(is_pretrain=False)
    seg_model = OverSegmentModelV3(is_pretrain=False)
    cls_model = OverClassifyModelV3(is_pretrain=False)
    return restore_model, seg_model, cls_model


def test_step(restore_model, seg_model, cls_model, bat, use_seg=False):
    bat_im, bat_filepath, bat_im_size = bat
    pad = 16
    ...
    final_h, final_w = bat_im_size[0][0].item(), bat_im_size[0][1].item()
    res_im = Image.fromarray(res)
    res_im = res_im.resize((final_w, final_h), Image.BILINEAR)
    res_im.save(
...
def main():
    ...
    restore_chk_path = 'checkpoint/v3/best_epoch_245/model_0.pd'
    seg_chk_path = 'checkpoint/v3/best_epoch_245/model_1.pd'
    cls_chk_path = 'checkpoint/v3/best_epoch_245/model_2.pd'
