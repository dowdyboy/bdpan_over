...
from bdpan_over.v6.dataset import OverTestDataset
from bdpan_over.v4.model import OverClassifyModelV4
from bdpan_over.v6.model import OverRestoreSegmentModelV6, over_model_b2
...
def build_model():
    restore_model = over_model_b2(1024, encoder_pretrain=None, use_sigmoid=False, mlp_act=False)
    ...
    for i in range(0, h, step):
        for j in range(0, w, step):
            ...
            if cls_pred == 0:
                ...
                restore_pred, seg_pred = restore_model(clip)
                seg_im = paddle.argmax(seg_pred, axis=1)
                seg_im = seg_im.astype('float32')
                res[:, :, i + pad:i + step + pad, j + pad:j + step + pad] = restore_pred[:, :, pad:-pad, pad:-pad]
                res_seg[:, i + pad:i + step + pad, j + pad:j + step + pad] = seg_im[:, pad:-pad, pad:-pad]

    print(f'{cls_count} / {all_count}')
    res = res[:, :, pad:-pad, pad:-pad]
    res_seg = res_seg[:, pad:-pad, pad:-pad]
    bat_im = bat_im[:, :, pad:-pad, pad:-pad]
...
def main():
    ...
    restore_chk_path = 'checkpoint/v6/best_epoch_68/model_0.pd'
