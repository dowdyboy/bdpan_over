...
class OverModelEncoderV3(nn.Layer):
    ...
    def __init__(self,
        ...
        super(OverModelEncoderV3, self).__init__()
        self.scales = scales
...
class OverModelDecoderV3(nn.Layer):

    def __init__(self, feats_in_channel=[24, 24, 48, 96, 192], out_channel=16, ):
        super(OverModelDecoderV3, self).__init__()
        ...
        self.final_up_conv = UpSampleConvLayer(feats_in_channel[-1], out_channel)
...
class OverModelHeadV3(nn.Layer):

    def __init__(self, in_channel=16, num_classes=2, use_drop=True,):
        super(OverModelHeadV3, self).__init__()
...
class OverClassifyModelV3(nn.Layer):

    def __init__(self, num_classes=2, is_pretrain=True):
        super(OverClassifyModelV3, self).__init__()
        self.shuffle50_net = ShuffleNetV2(scale=1.0, act='relu', num_classes=num_classes, with_pool=True)
        if is_pretrain:
            net_weight_path = 'checkpoint/pretrain/shufflenet_v2_x1_0.pdparams'
            net_weight = paddle.load(net_weight_path)
...
class OverSegmentModelV3(nn.Layer):

    def __init__(self, is_pretrain=True):
        super(OverSegmentModelV3, self).__init__()
        self.encoder = OverModelEncoderV3(
            scales=['0.25', '0.33', ],
            feats_out_channel=[24, 24, 32, 64, 128],
            num_classes=2,
            is_pretrain=is_pretrain,
        )
        self.decoder = OverModelDecoderV3(
            feats_in_channel=[24, 24, 32, 64, 128],
            out_channel=16,
        )
        self.head = OverModelHeadV3(
            in_channel=16,
            num_classes=2,
            use_drop=True,
        )
...
class OverRestoreModelV3(nn.Layer):

    def __init__(self, is_pretrain=True):
        super(OverRestoreModelV3, self).__init__()
        self.encoder = OverModelEncoderV3(
            scales=['0.5', '1.0', ],
            feats_out_channel=[24, 32, 64, 128, 256],
            num_classes=2,
            is_pretrain=is_pretrain,
        )
        self.decoder = OverModelDecoderV3(
            feats_in_channel=[24, 32, 64, 128, 256],
            out_channel=16,
        )
        self.head = OverModelHeadV3(
            in_channel=16,
...
class OverModelV3(nn.Layer):

    def __init__(self):
        super(OverModelV3, self).__init__()
        self.cls_model = OverClassifyModelV3()
        self.seg_model = OverSegmentModelV3()
        self.restore_mode = OverRestoreModelV3()
