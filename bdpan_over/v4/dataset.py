...
class OverDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.resize = resize  # resize rate
        self.no_limit_patch_pos_p = no_limit_patch_pos_p
    ...
    def _get_fuse_item(self):
        ...
        def calc_start_point(h, w, p_h, p_w, p_pos):
            if self.no_limit_patch_pos_p is not None and random.random() < self.no_limit_patch_pos_p:
                return random.randint(0, h - p_h), random.randint(0, w - p_w)
            else:
                if p_pos == 0:
    ...
    def _trans_resize(self, image_img, gt_img, mask_img):
        if self.resize is not None:
            ...
            m_w, m_h = mask_img.width, mask_img.height
            assert im_w == g_w and im_h == g_h and im_w == m_w and im_h == m_h
            new_h = int(im_h * self.resize)
            new_w = int(im_w * self.resize)
            image_img = F.resize(image_img, (new_h, new_w))
            gt_img = F.resize(gt_img, (new_h, new_w))
            mask_img = F.resize(mask_img, (new_h, new_w))
        return image_img, gt_img, mask_img
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_val:
            im, gt, mask = self._trans_resize(im, gt, mask)
...
class OverTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.resize is not None:
            new_h = int(im_h * self.resize)
            new_w = int(im_w * self.resize)
            im = F.resize(im, (new_h, new_w))
        return self.to_tensor(im), filepath, np.array([im_h, im_w], dtype=np.long)
