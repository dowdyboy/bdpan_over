...
class OverModelEncoderV5(nn.Layer):
    ...
    def __init__(self,
        ...
        super(OverModelEncoderV5, self).__init__()
        self.scales = scales
...
class OverModelDecoderV5(nn.Layer):

    def __init__(self, feats_in_channel=[24, 24, 48, 96, 192], out_channel=16, ):
        super(OverModelDecoderV5, self).__init__()
...
class OverModelHeadV5(nn.Layer):

    def __init__(self, in_channel=16, num_classes=2, use_drop=True,):
        super(OverModelHeadV5, self).__init__()
...
class OverClassifyModelV5(nn.Layer):

    def __init__(self, num_classes=2, is_pretrain=True):
        super(OverClassifyModelV5, self).__init__()
...
class OverRestoreSegmentModelV5(nn.Layer):

    def __init__(self, is_pretrain=True, use_sigmoid=False):
        super(OverRestoreSegmentModelV5, self).__init__()
        self.use_sigmoid = use_sigmoid
        self.encoder = OverModelEncoderV5(
            scales=['1.0', '1.5', '2.0'],
            feats_out_channel=[24, 24, 176, 352, 704],
            num_classes=2,
            is_pretrain=is_pretrain,
        )
        self.decoder = OverModelDecoderV5(
            feats_in_channel=[24, 24, 176, 352, 704],
            out_channel=16,
        )
        self.restore_head = OverModelHeadV5(
            in_channel=16,
            num_classes=3,
            use_drop=False,
        )
        self.seg_head = OverModelHeadV5(
            in_channel=16,
            num_classes=2,
            use_drop=True,
        )
    ...
    def forward(self, x):
        ...
        out_restore = self.restore_head(fuse_dec_out)
        out_seg = self.seg_head(fuse_dec_out)
        if self.use_sigmoid:
